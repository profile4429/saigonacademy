    ...
    <div class="container">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="text-center mb-0">Trans Programs</h5>
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('transPrograms')}}" enctype="multipart/form-data">
                    @csrf
                            ...
                            <select class="form-control" name="language_id_from" id="language_id_from" required disabled>
                                @foreach ($language as $item)
                                    <option value="{{ $item->id }}"
                                        {{ !empty($programs) && $programs->language_id == $item->id ? 'selected' : '' }}>
                                        {{ $item->title }}
                                    </option>
                                @endforeach
                            </select>
                            <?php $language_id_from = $programs->language_id ?? null; ?>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-control" id="description">{{$programs->description }}</textarea>
                    </div>
