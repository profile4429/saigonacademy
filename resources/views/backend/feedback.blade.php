    ...
    <div class="container-fluid">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="text-center mb-0">Add Feedback</h5>
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('addFeedback') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input name="career" required type="text" class="form-control" id="career" value="">
                    </div>
                    <div class="form-group">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Detele Feedback</h5>
                ...
                <div class="modal-footer">
                    <input type="hidden" id="idFeedback" value="">
                        ...
                        <tbody>
                            @foreach ($feedback as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('showdataFeedback') }}?id={{ $item->id }}"><button
                                                class="btn btn-warning">Edit</button></a>
                                        <button type="button" class="btn btn-danger btnDelete"
                                            data-id="{{ $item->id }}">Delete</button>
                                        
                                            @php
                                            $feedbacklocale = App\Models\feedbacklocale::where('vi', $item->id)
                                                ->orWhere('en', $item->id)
                                                ->first();
                                            if ($feedbacklocale && (($feedbacklocale->vi == $item->id && $feedbacklocale->en != null) || ($feedbacklocale->en == $item->id && $feedbacklocale->vi != null))) {
                                                $hide_translate = true;
                                            } else {
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', ".btnDel", function() {
                let id = $('#idFeedback').val();

                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: '{{ route('deleteFeedback') }}',
                    data: {
                        'id': id,
                        '_token': '{{ csrf_token() }}'
                    },
