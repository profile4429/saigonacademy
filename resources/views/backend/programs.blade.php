    ...
    <div class="container-fluid">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="text-center mb-0">Add Programs</h5>
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('addPrograms') }}" enctype="multipart/form-data">
                    @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Detele Programs</h5>
                ...
                <div class="modal-footer">
                    <input type="hidden" id="idPrograms" value="02">
                        ...
                        <tbody>
                            @foreach ($programs as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('showdataPrograms') }}?id={{ $item->id }}"><button
                                                class="btn btn-warning">Edit</button></a>
                                        <button type="button" class="btn btn-danger btnDelete"
                                            data-id="{{ $item->id }}">Delete</button>
                                            @php
                                            $programslocale = App\Models\programslocale::where('vi', $item->id)
                                                ->orWhere('en', $item->id)
                                                ->first();
                                            if ($programslocale && (($programslocale->vi == $item->id && $programslocale->en != null) || ($programslocale->en == $item->id && $programslocale->vi != null))) {
                                                $hide_translate = true;
                                            } else {
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', ".btnDel", function() {
                let id = $('#idPrograms').val();

                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: '{{ route('deletePrograms') }}',
                    data: {
                        'id': id,
                        '_token': '{{ csrf_token() }}'
                    },
