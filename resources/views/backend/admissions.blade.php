    ...
    <div class="container-fluid-fluid">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="text-center mb-0">Add Admissions</h5>
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('addAdmissions') }}" enctype="multipart/form-data">
                    @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Detele Admissions</h5>
                ...
                <div class="modal-footer">
                    <input type="hidden" id="idAdmissions" value="">
                        ...
                        <tbody>
                            @foreach ($admissions as $item)
                                <tr>
                                    ...
                                    @endforeach
                                    <td>{{ $item->title }}</td>
                                    <td><img class="card-img-top" style="height: 150px; width:300px; object-fit: cover;"
                                        src="{{ asset('/images/' . $item->image) }}" alt=""></td>
                                    <td>{!! $item->description !!}</td>
                                    <td>
                                        <a href="{{ route('showdataAdmissions') }}?id={{ $item->id }}"><button
                                                class="btn btn-warning">Edit</button></a>
                                        <button type="button" class="btn btn-danger btnDelete"
                                            data-id="{{ $item->id }}">Delete</button>
                                            @php
                                            $admissionslocale = App\Models\admissionslocale::where('vi', $item->id)
                                                ->orWhere('en', $item->id)
                                                ->first();
                                            if ($admissionslocale && (($admissionslocale->vi == $item->id && $admissionslocale->en != null) || ($admissionslocale->en == $item->id && $admissionslocale->vi != null))) {
                                                $hide_translate = true;
                                            } else {
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', ".btnDel", function() {
                let id = $('#idAdmissions').val();

                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: '{{ route('deleteAdmissions') }}',
                    data: {
                        'id': id,
                        '_token': '{{ csrf_token() }}'
                    },
